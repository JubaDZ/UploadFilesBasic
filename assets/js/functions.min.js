/*! Functions v0.9.8| (c) 2013-2020 onexite and other contributors | MIT license */
...
function ShowLinks( FileNum ) {
	...
	{
		$("#label_view").html('<a target="_blank" href=""></a>');
		$('#view').val('');
		$('#view_tr').hide();
	}
	...
	if(Direct4ImageOnly)
	{
	 ...
	 {
		 $("#label_directlink").html('<a target="_blank" href=""></a>'); 
		 $('#directlink').val(''); 
		 $('#direct_tr').hide();
	 }
...
function pageUserFiles(id) {
  redirect('?files&user='+id);
};
...
function LoadComments(num) {
    ...
    $.get("ajax/index.php?comments="+fileid+"&currentpage=" + num, function(data, status) {
        if (status == 'success') {
			...
			$('#page-selection').bootpag({total: data.totalpages,maxVisible: _maxVisible}).on("page", function(event, num) {
				$("#all_comment").html('<hr><i class="icon-spin4 animate-spin"></i> ' + Loading + ' ( <code>1</code> / <code>' + data.totalpages + '</code> ) ...<hr>'); 
				LoadComments(num);
				});
...
function LoadTable(num,usr) {
	$('#select_all').prop('checked',false);
	$('#span_select_all').hide();
	$("#button-selection").hide(); 
    $.get("ajax/index.php?files&currentpage=" + num+"&user=" + usr, function(data, status) {
        if (status == 'success') {
...
bootbox.confirm({
    ...
    buttons: {confirm: {label: _Yes},cancel: {label: _No}},callback: function (Confirmed) {  
		...
		$.get("ajax/index.php?delete="+deleteid+"&id="+id, function(data, status){
			if (status == 'success') LoadTable(_page,Public_user_id);
		});
		}
...
window.onload = function() {
    ...
    var 
        ...
        // false/default = do not remove
			...
			if(Extensions_not_allowed.indexOf(ext) != -1)
			{					
	        //	 ShowMessage(ExtErrMsg+' *** '+ext);     	
			}
			if(IsStatsPanel)
				{
        ...
        onExtError: function( filename, extension ) {
            ShowMessage(ExtErrMsg+' - '+extension);
        },
          ...
          beforeSend: function() {
           ...
           success: function (data) {
               if(data['success_msg']) {
				    ...
				    $('#page-selection').bootpag({ total: data.totalpages ,maxVisible: _maxVisible}); 
